(function($){
   ...
   var private_methods = {
       ...
       price:function(date,opts){
           ...
           if(len>0){
               for(var i= 0;i<len;i++){
                   var targetdate=opts.settingdata[i].currDate.substring(0,10);
                   if(date==targetdate){
                	   if(!opts.settingdata[i].currPrice){
                		   pricetext="<span class='price' data-childstock='"+opts.settingdata[i].childStock+"' data-stock='"+opts.settingdata[i].leftSum+"' data-childPrice='"+opts.settingdata[i].childPrice+"'><strong>暂无价格</strong></span>";
                	   }else{
                		   pricetext="<span class='price' data-childstock='"+opts.settingdata[i].childStock+"' data-stock='"+opts.settingdata[i].leftSum+"' data-childPrice='"+opts.settingdata[i].childPrice+"'><em>￥</em><strong>"+opts.settingdata[i].currPrice+"</strong></span>";
                	   }



                       if(!opts.settingdata[i].leftSum||opts.settingdata[i].leftSum=='0'||!opts.settingdata[i].currPrice){
                           if(opts.multiselect){
                               if(!opts.settingdata[i-1].leftSum||opts.settingdata[i-1].leftSum=='0'||!opts.settingdata[i-1].currPrice){
                                   show=false;
                               }else{
                                   show=true;
                               }
                       ...
                       stock=opts.settingdata[i].leftSum;
                   }
       ...
       settingdate:function(data){
           ...
           $.each(data,function(i){
               date.push(data[i].currDate.substring(0,10));
           });
           return date;
       }
    };
                ...
                if(typeof opts.selecteday!='string'){
                    if(today == itemDay){
                        if($.inArray(today,private_methods.settingdate(opts.settingdata))>-1){
                            ...
                            if(this.ieDate(opts.selecteday[0])<=this.ieDate(itemDay)&&this.ieDate(opts.selecteday[1])>=this.ieDate(itemDay)){
                                html += " data-day='"+(j + 1)+"' class='selecteds'>";
                            }else{
                                html += " data-day='"+(j + 1)+"' class>";
                            }
                        }
                    ...
                    if(today == itemDay){
                        ...
                        if(pricedata[2]!=null && pricedata[2]!=0 ){
                            ...
                            html += "<span class='date-title'><i>今天</i><em>暂无库存</em></span><p class='calendar-explian'>"+pricedata[0]+"</p>";
                        }else if(pricedata[2]!=null && pricedata[2]==0 ){
