...
function hoteltotalprice(){
    var price = $("#avg").text();
    var val = $("#amount").val();
    $("#totla").text(operation.accSub($("#yhje").text(),operation.accMul(price,val)));
}
...
function dilogNav(){
    $(document).ready(function(){
        ...
        $(window).scroll(function(){
            ...
            if(scroll_top > 500){
                $(".stairslist").addClass('show');
            }else{
                $(".stairslist").removeClass('show');
            }
            if(scroll_top < 400){
                $(".fast-bar").css('display','none');
                $("#qidian_wpa_9").css('display','none');
            }else{
                $(".fast-bar").css('display','block');
                $("#qidian_wpa_9").css('display','block');
            }
        });
    });
}
